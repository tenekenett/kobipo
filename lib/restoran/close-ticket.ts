// Adisyon KAPANIŞ çekirdeği — `/api/restoran/adisyonlar/[id]/kapat`ın iş mantığı.
//
// Gövdeler uçtan OLDUĞU GİBİ taşındı (davranış değişmedi). Neden: fiş + tahsilat +
// kapanış ÖKC webhook'undan (oturumsuz) da yürüyebilsin — bkz. lib/api/write-actor.ts,
// lib/restoran/close-with-receipt.ts, docs/okc/ASAMA1-KOBIPO.md A5.
import { NextResponse } from "next/server"
import { resolveCompanyId } from "@/lib/company/resolve-company"
import { prisma } from "@/lib/db/prisma"
import { assertRestaurantModule, optionRecipeEffects, parseItemOptions, serializeTicket, ticketInclude } from "@/lib/restoran/tickets"
...
/** Kapanış gövdesini hazırlar (fiş payload'ı + sahipsiz fiş). `authorize` OKUMA yetkisidir. */
export async function prepareTicketClose(
  ...
  try {
    ...
    assertRestaurantModule(await authorize(companyId))
...
/** Adisyonu fişe bağlayıp (ya da fişsiz) KAPATIR; ikram/zayi stoğunu düşer. */
export async function closeTicket(
  ...
  try {
    ...
    assertRestaurantModule(await actor.authorize(companyId))
    ...
    //
    // Masada AYRILMIŞ başka hesap hâlâ açıksa masa boşalmadı: damga ancak SON hesap
    // kapanınca basılır (lib/restoran/split.ts — bir masada birden çok hesap).
    if (ticket.tableId) {
      const stillOpen = await prisma.restaurantTicket.count({
        where: { companyId, tableId: ticket.tableId, status: "OPEN" },
      })
      if (stillOpen === 0) {
