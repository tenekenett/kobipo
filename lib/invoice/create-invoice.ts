// Fatura / fiş OLUŞTURMA çekirdeği — `POST /api/e-donusum/invoices`in iş mantığı.
//
// NEDEN AYRI DOSYA (docs/okc/ASAMA1-KOBIPO.md A5): fişi bugüne kadar yalnız oturumu
// olan bir istek kesebiliyordu. ÖKC bulut entegrasyonunda (Aşama 2) "ödendi" haberi
// yazarkasadan webhook ile SUNUCUYA gelir; ekran açık olmayabilir (garson masada
// cihazdan öder). Aynı fiş yolu — stok düşümü, reçete genişletme, cari, muhasebe —
// oradan da yürüyebilsin diye mantık oturumdan ayrıldı. Tek fiş yolu kuralı korunur:
// uç ve webhook AYNI fonksiyonu çağırır, ikinci bir satış yolu yoktur.
//
// Gövde uçtan OLDUĞU GİBİ taşındı (davranış değişmedi); yalnız iki bağımlılık
// dışarıdan verilir: `actor.userId` (createdBy) ve `actor.authorize` (yazma yetkisi —
// uçta `ensureCompanyWrite`, aynı noktada çağrılır). Bkz. lib/api/write-actor.ts, lib/api/session-actor.ts
...
import { computeInvoiceTotals, documentColumnPrecision, resolveLineDiscount } from "@/lib/invoice/document-totals"
...
/**
 * Fatura/fiş oluşturur ve HTTP yanıtını döndürür (uç bunu aynen iletir). `readBody`
 * try içinde okunur: bozuk JSON eskisi gibi 500'e düşer.
 */
export async function createInvoiceFromBody(
  ...
  try {
    ...
    await actor.authorize(companyId)
