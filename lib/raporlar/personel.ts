/**
 * Personel (İK) raporu hesabı.
 *
 * `app/api/raporlar/personel/route.ts`ten ayıklandı — dışa aktarma ucu da aynı
 * fonksiyonu çağırır.
 */
...
export async function computeHrReport(args: {
  companyId: string
  year: number
}): Promise<HrReportResult> {
  const { companyId, year } = args
