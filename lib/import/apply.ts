...
export async function applyProductRow(
  ...
  await prisma.product.create({
    data: {
      ...
      minStockLevel: get("minstocklevel") ? parseDecimal(get("minstocklevel"), 0) : null,
  ...
  await model.create({
    data: {
      ...
      paymentDueDays: get("paymentduedays")
        ? Math.max(0, Math.trunc(parseDecimal(get("paymentduedays"), 0)))
        : null,
      openingBalanceAmount: parseDecimal(get("openingbalance"), 0),
