...
/**
 * Sistem-admin'in fatura müdahale ucu.
 *
 *   action: "issue" (varsayılan) → faturayı kes / kalınan yerden devam et.
 *   action: "void"               → kesilmiş faturayı geri al.
 *
 * "issue" idempotenttir ve kapılara TABİDİR: test siparişi ya da faturalandırma
 * kapalıysa elle tetiklemek de belge kesmez — kapıların anlamı budur.
 */
export async function POST(
  ...
  try {
    ...
    const order = await prisma.kontorOrder.findUnique({ where: { id }, select: { id: true } })
    ...
    if (action === "void") {
      ...
      if (!res.ok) {
        return NextResponse.json(
          {
            error: res.needsManual ? res.instruction : res.error,
            needsManual: res.needsManual,
          },
          // needsManual bir sunucu hatası değil, mevzuat sınırı: 409.
    ...
    const res = await issueSalesInvoiceForOrder({ kind: "KONTOR", orderId: id })
    if (!res.ok) {
      return NextResponse.json(
    ...
    console.error("kontor order invoice action error:", error)
