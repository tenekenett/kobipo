...
export const GET = withApiErrors(async function GET(
  ...
  try {
    ...
    resolvedParams.id = await resolveSlugId("customer", resolvedParams.id, await resolveCompanyId(new URL(request.url).searchParams.get("companyId")))
    ...
    const [allInvoices, allTransactions, allChecks, allNotes, convertedReceipts] = await Promise.all([
      ...
      // hangi faturaya dönüştüğü belirtilerek). Ekonomik etki artık faturada olduğu
...
export const PUT = withApiErrors(async function PUT(
  ...
  try {
    ...
    resolvedParams.id = await resolveSlugId("customer", resolvedParams.id, requestedCompanyId)
    const customer = await prisma.customer.findUnique({
      where: { id: resolvedParams.id },
    })
    ...
    if (requestedCompanyId && customer.companyId !== requestedCompanyId) {
      return NextResponse.json({ error: "Customer not found" }, { status: 404 })
    }
    ...
    // İkizin AYNADAKİ hâli: müşteri kartından tedarikçi doğuyor/siliniyorsa
    // "Tedarikçi" yazma yetkisi de şart. Koşullar aşağıdaki işlemin dallarıyla
    // birebir aynı (bkz. lib/cari/dual-role-access.ts).
    {
      const nextIsAlsoSupplier =
        isAlsoSupplier !== undefined ? Boolean(isAlsoSupplier) : customer.isAlsoSupplier
      const willDelete = isAlsoSupplier === false && Boolean(customer.linkedSupplierId)
      const willCreate = nextIsAlsoSupplier && !customer.linkedSupplierId
