...
export default function SatisSiparisPage() {
  ...
  async function fetchOrders() {
    ...
    setIsLoading(true)
    try {
      const res = await fetch(`/api/siparis?companyId=${companyId}&type=SALES`)
  ...
  const filtered = orders.filter((o) => {
    ...
    return (
      ...
      (o.customer?.name || "").toLocaleLowerCase("tr-TR").includes(term)
    )
  })
      ...
      <Card>
                        ...
                        {lines.map((row, index) => (
                          <div key={index} className="grid gap-2 border-b pb-3 last:border-0 last:pb-0 sm:grid-cols-12">
                            <div className="sm:col-span-3">
                              <Label className="text-xs text-muted-foreground">Ürün (opsiyonel)</Label>
                              <Select
        ...
        <CardContent>
          ...
          {!isLoading && orders.length === 0 && (
            <div className="text-sm text-muted-foreground">Henüz satış siparişi yok.</div>
          )}
