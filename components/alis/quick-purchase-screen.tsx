...
export function QuickPurchaseScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [prodRes, suppRes, accRes, whRes, stockRes, catRes] = await Promise.all([
        ...
        if (!cancelled && prodRes.ok) {
          ...
          setProducts(
            (Array.isArray(data) ? data : []).map((p: any) => ({
              ...
              name: p.name,
              code: p.code,
              // Alış bağlamı: satır birim fiyatı ürünün ALIŞ fiyatından gelsin.
              salePrice: p.purchasePrice != null ? Number(p.purchasePrice) : null,
              vatRate: Number(p.vatRate) || 20,
              unit: p.unit,
            }))
          )
        }
        if (!cancelled && suppRes.ok) {
          const data = await suppRes.json()
          const items = Array.isArray(data) ? data : data?.items ?? []
          setSuppliers(
            items.map((s: any) => ({ id: s.id, name: s.name, taxNumber: s.taxNumber }))
          )
        }
        ...
        console.error("Hızlı alış verileri yüklenemedi:", error)
      }
    }
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      if (!invoiceRes.ok) {
        throw new Error(invoice?.error || "Alış faturası oluşturulamadı")
      }

      // Ödeme (açık hesap değilse ve tutar > 0)
      ...
      toast({
        ...
        description: `${invoice.invoiceNo ?? "Fiş"} oluşturuldu${
          isCredit ? " (açık hesap)" : ` • ${currency(totals.total)} ödendi`
        }`,
      })
      resetPurchase()
    } catch (error: any) {
      toast({
        title: "Hata",
        description: error?.message || "Alış tamamlanamadı",
        variant: "destructive",
      })
    } finally {
      setIsSubmitting(false)
            ...
            <CardContent>
              {cart.length === 0 ? (
                <div className="py-8 text-center text-muted-foreground">
                  <PackagePlus className="mx-auto mb-2 h-6 w-6 opacity-50" />
                  Boş — yukarıdan ürün ekleyin
                </div>
              ) : (
                <>
                  {/* Masaüstü: tablo */}
        ...
        <div className="space-y-4 lg:sticky lg:top-4 lg:self-start">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Opsiyonel — boş bırakılırsa serbest alış fişi</CardDescription>
            </CardHeader>
            <CardContent>
              <CounterpartyCombobox
                ...
                onSelect={(sel) =>
                  setSelectedSupplierId(sel && sel.kind === "supplier" ? sel.id : undefined)
                }
          ...
          {warehouses.length > 1 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Stok bu depoya girecek</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {accounts.length === 0 ? (
                      <p className="text-xs text-muted-foreground">
                        Tanımlı hesap yok — ödeme hesaba işlenmeyecek.
                      </p>
                    ) : (
